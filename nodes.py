...
class Doctype(Atom):
    ...
    def eval(self, hisp):
        from hisp import ConversionError
        from doctypes import doctypes
        import re
        for (ftype, dtypes) in doctypes.items():
            for (regex, dtype) in dtypes.items():
                regex = r'^%s$' % regex
                if re.match(regex, self.value):
                    hisp.setf(ftype)
                    return re.sub(regex, dtype, self.value)
...
class Elem(Node):
    ...
    def render(cls, hisp, tag, attrs, children=None):
        ...
        body = hisp.indent(hisp.join(children))
        return hisp.join(u'<%s>' % head, body, u'</%s>' % tag)
...
class Text(Node):
    ...
    def eval(self, hisp):
        return hisp.combine(*self.parts)
